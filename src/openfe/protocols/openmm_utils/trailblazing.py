...
class _InMemorySnapshotReporter:
    ...
    def describeNextReport(self, simulation: app.Simulation) -> tuple[int, bool, bool, bool, bool, bool]:
    ...
    def report(self, simulation: app.Simulation, state: openmm.State) -> None:
        self.snapshots.append(
            _StateSnapshot(
                ...
                box_vectors_nm=state.getPeriodicBoxVectors(asNumpy=True).value_in_unit(ommunit.nanometer),
            )
        )
...
class TrailblazingMixin:
    ...
    def _get_settings(self):
        ...
        settings["equil_simulation_settings"] = prot_settings.solvent_equil_simulation_settings.unfrozen_copy()
        settings["equil_output_settings"] = prot_settings.solvent_equil_output_settings.unfrozen_copy()
    ...
    def _lambda_converter(self, lambda_value: float) -> dict[str, float]:
        ...
        return {
            ...
            "lambda_sterics": np.heaviside(lambda_value -0.5, 0.5) + 2 * lambda_value * np.heaviside(0.5 - lambda_value, 0.5),
        }
    ...
    def _get_reduced_potential(self, simulation: app.Simulation, settings) -> Quantity:
        ...
        return potential_energy / self._kbT + (pressure * volume * ommunit.AVOGADRO_CONSTANT_NA) / self._kbT

    def _get_snapshot_energies(self, simulation: app.Simulation, snapshots: list[_StateSnapshot], settings) -> list[Quantity]:
        energies = []
    ...
    def _sample_state(self, simulation: app.Simulation, lambda_value: float, positions: ommunit.Quantity, settings, equil_steps_nvt: int | None, equil_steps_npt: int, prod_steps_npt: int) -> tuple[list[Quantity], list[_StateSnapshot]]:
        """
        ...
        prod_snapshots = [snapshot for snapshot in snapshot_reporter.snapshots if snapshot.step > prod_start_step]
        ...
        potential_energies = state_data["Potential Energy (kJ/mole)"].values * ommunit.kilojoule_per_mole
        volumes = state_data["Box Volume (nm^3)"].values * ommunit.nanometer ** 3
        pressure = to_openmm(settings["thermo_settings"].pressure)
        simulated_energies = potential_energies / self._kbT + (pressure * volumes * ommunit.AVOGADRO_CONSTANT_NA) / self._kbT
    ...
    def _reweight_samples(self, simulation: app.Simulation, snapshots: list[_StateSnapshot], reweight_lambda_value: float, settings) -> list[Quantity]:
        """
    ...
    def _run_trailblazing_method(self, system: openmm.System, topology: app.Topology, positions: ommunit.Quantity, thermodynamic_distance: float = 1.0, distance_tolerance: float = 0.05) -> list[float]:
        """
        ...
        self._kbT = (ommunit.MOLAR_GAS_CONSTANT_R * to_openmm(settings["thermo_settings"].temperature))
        ...
        while optimal_lambda[-1] < 1.0:
            ...
            while abs(std_energy - thermodynamic_distance) > distance_tolerance and not (current_lambda == 1.0 and std_energy < thermodynamic_distance):
                if np.isclose(std_energy, 0.0):
                    ...
                    derivative_std_energy = ((std_energy - old_std_energy) / (current_lambda - old_lambda))
                    old_lambda = current_lambda
                ...
                logger.info(f"Trailblazing: simulated_lambda={optimal_lambda[-1]}, current_lambda={current_lambda:.3f}, std_energy={std_energy:.3f}, target={thermodynamic_distance:.3f}, tolerance={distance_tolerance:.3f}")
                print(f"Trailblazing: simulated_lambda={optimal_lambda[-1]}, current_lambda={current_lambda:.3f}, std_energy={std_energy:.3f}, target={thermodynamic_distance:.3f}, tolerance={distance_tolerance:.3f}")


            # add the value to the optimal lambda schedule
            optimal_lambda.append(current_lambda)
            state_stds.append(std_energy)
            logger.info(f"Trailblazing: accepted lambda={current_lambda:.3f}, std_energy={std_energy:.3f}, target={thermodynamic_distance:.3f}, tolerance={distance_tolerance:.3f}")
            print(f"Trailblazing: accepted lambda={current_lambda:.3f}, std_energy={std_energy:.3f}, target={thermodynamic_distance:.3f}, tolerance={distance_tolerance:.3f}")
            # todo
