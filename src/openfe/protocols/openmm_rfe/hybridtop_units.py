...
class HybridTopologySetupUnit(gufe.ProtocolUnit, HybridTopologyUnitMixin):
    ...
    def _handle_net_charge(
        ...
        distance_cutoff : Quantity
        forcefield : openmm.app.ForceField
            The forcefield used to parameterize the system. Needed to
            obtain ion parameters when no ions are present in the topology.
        """
    ...
    def _get_omm_objects(
        ...
        if settings["alchemical_settings"].explicit_charge_correction:
            # The forcefield from the system generator contains all the
            # ion templates we need. We use state A's generator, but both
            # states share the same forcefield parameters.
