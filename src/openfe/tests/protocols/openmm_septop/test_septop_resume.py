...
def protocol_settings():
    ...
    settings.complex_simulation_settings.production_length = 100 * offunit.picosecond
    settings.solvent_simulation_settings.equilibration_length = 50 * offunit.picosecond
    settings.solvent_simulation_settings.production_length = 100 * offunit.picosecond
    ...
    settings.complex_output_settings.checkpoint_interval = 50 * offunit.picosecond
    settings.solvent_output_settings.checkpoint_interval = 50 * offunit.picosecond
...
class TestCheckpointResuming:
    ...
    def test_create_files(self, protocol_dag):
        ...
        setup_results = setup_unit.run(
            scratch_basepath=pathlib.Path("setup"),
            shared_basepath=pathlib.Path("setup"),
            dry=True,
        )

        pdb_file = openmm.app.pdbfile.PDBFile(str(setup_results["topology"]))
        solv_sampler = sim_unit.run(
            setup_results["alchem_restrained_system"],
            pdb_file,
            setup_results["selection_indices"],
            scratch_basepath=pathlib.Path("simulation"),
            shared_basepath=pathlib.Path("simulation"),
        )
