...
class TestProtocolResult:
    ...
    def test_get_estimate(self, protocolresult):
        ...
        assert est
        assert est.m == pytest.approx(-20.76, abs=0.01)
    ...
    def test_get_uncertainty(self, protocolresult):
        ...
        assert est
        assert est.m == pytest.approx(1.18, abs=0.01)
        assert isinstance(est, offunit.Quantity)
        assert est.is_compatible_with(offunit.kilojoule_per_mole)
    ...
    def test_restraint_geometry(self, protocolresult):
        ...
        assert geom[0].host_atoms == [911, 912, 913]
        assert pytest.approx(geom[0].r_aA0) == 1.089738 * offunit.nanometer
        assert pytest.approx(geom[0].theta_A0) == 1.211829 * offunit.radian
        assert pytest.approx(geom[0].theta_B0) == 1.435830 * offunit.radian
        assert pytest.approx(geom[0].phi_A0) == 2.313956 * offunit.radian
        assert pytest.approx(geom[0].phi_B0) == 1.843409 * offunit.radian
        assert pytest.approx(geom[0].phi_C0) == -0.1554106 * offunit.radian
