...
def test_validate_solvent_multiple_solvent(benzene_modifications):
    ...
    state = openfe.ChemicalSystem(
        {"A": benzene_modifications["toluene"], "S": openfe.SolventComponent(), "S2": openfe.SolventComponent()},
    )
...
def test_not_water_solvent(benzene_modifications):
    ...
    state = openfe.ChemicalSystem({"A": benzene_modifications["toluene"], "S": openfe.SolventComponent(smiles="C")})
...
class TestFEAnalysis:
    ...
    def test_free_energies(self, analyzer):
        ...
        assert_allclose(
            ...
            np.array([0.08988764, 0.191011, 0.292135, 0.393258, 0.494382, 0.595506, 0.696629, 0.797753, 0.898876, 1.0]),
            rtol=1e-04,
        )
        assert_allclose(
        ...
        assert_allclose(
            ...
            np.array(
                [0.07471, 0.052914, 0.041508, 0.036613, 0.032827, 0.030489, 0.028154, 0.026529, 0.025284, 0.023968],
            ),
            rtol=1e-04,
        )
        assert_allclose(
        ...
        assert_allclose(
            ...
            np.array(
                [0.081209, 0.055975, 0.044693, 0.038691, 0.034603, 0.031894, 0.029417, 0.027082, 0.025316, 0.023968],
            ),
            rtol=1e-04,
        )
...
class TestSystemCreation:
    def test_system_generator_nosolv_nocache(self, get_settings):
        ...
        periodic_kwargs = {"nonbondedMethod": app.PME, "nonbondedCutoff": 1.0 * ommunit.nanometer}
        nonperiodic_kwargs = {
    ...
    def test_system_generator_solv_cache(self, get_settings):
        ...
        generator = system_creation.get_system_generator(ffsets, thermosets, intsets, Path("./db.json"), True)

        # Check barostat conditions
        assert isinstance(generator.barostat, MonteCarloBarostat)

        pressure = ensure_quantity(
            generator.barostat.getDefaultPressure(),
            "openff",
        )
        temperature = ensure_quantity(
            generator.barostat.getDefaultTemperature(),
            "openff",
        )
    ...
    def test_get_omm_modeller_complex(self, T4_protein_component, benzene_modifications, get_settings):
        ffsets, intsets, thermosets = get_settings
        ...
        assert_equal(comp_resids[openfe.SolventComponent()], np.linspace(165, len(resids) - 1, len(resids) - 165))
    ...
    def test_get_omm_modeller_ligand_no_neutralize(self, get_settings):
        ...
        system = generator.create_system(model.topology, molecules=[offmol])
...
class TestOFFPartialCharge:
    ...
    def test_am1bcc_reference(
        ...
        assert_allclose(am1bcc_ref_charges[ref_key], uncharged_mol.partial_charges, rtol=1e-4)

    def test_nagl_import_error(self, monkeypatch, uncharged_mol):
        monkeypatch.setattr(sys.modules["openfe.protocols.openmm_utils.charge_generation"], "HAS_NAGL", False)
    ...
    def test_espaloma_import_error(self, monkeypatch, uncharged_mol):
        monkeypatch.setattr(sys.modules["openfe.protocols.openmm_utils.charge_generation"], "HAS_ESPALOMA", False)
