...
@pytest.fixture
def afe_solv_protocol_json() -> str:
    """
    Serialized AFE solvation protocol (CN in water) generated
    at commit hash cfcc924de88fd28f775796b173807bc84ddab95d
    """
    d = resources.files(
        "openfe.tests.data.serialization.protocol_migrations.afe"
    )
    fname = "serialized_protocol.json"

    with open((d / fname).as_posix() , 'r') as f:  # type: ignore
        return f.read()


@pytest.fixture
def afe_solv_dag_json() -> str:
    """
    Serialized AFE solvation protocol DAG (CN in water) generated
    at commit hash cfcc924de88fd28f775796b173807bc84ddab95d
    """
    d = resources.files(
        "openfe.tests.data.serialization.protocol_migrations.afe"
    )
    fname = "serialized_dag.json"
