...
class SlowTests:
    ...
    def _modify_slow(items, config):
        msg = "need --runslow pytest cli option or the environment variable " "`OFE_SLOW_TESTS` set to `True` to run"
    ...
    def pytest_collection_modifyitems(self, items, config):
        if config.getoption("--integration") or os.getenv("OFE_INTEGRATION_TESTS", default="false").lower() == "true":
            return
        elif config.getoption("--runslow") or os.getenv("OFE_SLOW_TESTS", default="false").lower() == "true":
...
def am1bcc_ref_charges():
    ref_chgs = {
        "ambertools": [0.146957, -0.918943, 0.025557, 0.025557, 0.025557, 0.347657, 0.347657] * unit.elementary_charge,
        "openeye": [0.14713, -0.92016, 0.02595, 0.02595, 0.02595, 0.34759, 0.34759] * unit.elementary_charge,
        "nagl": [0.170413, -0.930417, 0.021593, 0.021593, 0.021593, 0.347612, 0.347612] * unit.elementary_charge,
        "espaloma": [0.017702, -0.966793, 0.063076, 0.063076, 0.063076, 0.379931, 0.379931] * unit.elementary_charge,
    }
    return ref_chgs
