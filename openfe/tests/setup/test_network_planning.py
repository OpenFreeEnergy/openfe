...
def test_radial_network(atom_mapping_basic_test_files, toluene_vs_others, as_list):
    ...
    assert all((central_ligand_name in {mapping.componentA.name, mapping.componentB.name}) for mapping in network.edges)
...
def test_generate_maximal_network(toluene_vs_others, with_progress, with_scorer, extra_mapper):
    toluene, others = toluene_vs_others
    if extra_mapper:
...
def test_minimal_spanning_network_regression(minimal_spanning_network):
    ...
    edge_ids = sorted((edge.componentA.name, edge.componentB.name) for edge in minimal_spanning_network.edges)
    ref = sorted(
        [
...
def test_redundant_vs_spanning_network(minimal_redundant_network, minimal_spanning_network):
    ...
    assert 2 * len(minimal_spanning_network.edges) == len(minimal_redundant_network.edges)


def test_minimal_redundant_network_edges(minimal_redundant_network):
    # issue #244, this was previously giving non-reproducible (yet valid)
    # networks when scores were tied.
    edge_ids = sorted((edge.componentA.name, edge.componentB.name) for edge in minimal_redundant_network.edges)
    ref = sorted(
        [
            ("1,3,7-trimethylnaphthalene", "2,6-dimethylnaphthalene"),
...
def test_network_from_indices(atom_mapping_basic_test_files):
    ...
    actual_edges = {(edges[0].componentA, edges[0].componentB), (edges[1].componentA, edges[1].componentB)}
