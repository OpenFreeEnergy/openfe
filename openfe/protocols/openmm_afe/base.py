...
from openfe.protocols.openmm_utils.omm_settings import BasePartialChargeSettings, SettingsBaseModel
from openfe.utils import without_oechem_backend

from ..openmm_utils import charge_generation, multistate_analysis, settings_validation, system_creation
...
class BaseAbsoluteUnit(gufe.ProtocolUnit):
    ...
    @staticmethod
    def _pre_minimize(system: openmm.System, positions: omm_unit.Quantity) -> npt.NDArray:
        """
    ...
    def _get_lambda_schedule(self, settings: dict[str, SettingsBaseModel]) -> dict[str, npt.NDArray]:
        """
    ...
    def _get_integrator(
        ...
        """
        steps_per_iteration = settings_validation.convert_steps_per_iteration(simulation_settings, integrator_settings)
    ...
    def run(self, dry=False, verbose=True, scratch_basepath=None, shared_basepath=None) -> dict[str, Any]:
        ...
        sampler_states, cmp_states = self._get_states(alchem_system, positions, settings, lambdas, solv_comp)
        ...
        try:
            ...
            energy_ctx_cache, sampler_ctx_cache = self._get_ctx_caches(settings["engine_settings"])
