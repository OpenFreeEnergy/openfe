...
class HybridTopologyFactory:
    ...
    * Implement omitted terms (this was not available in the original class).
    ...
    def __init__(
        ...
        has_nonbonded_force = (
            "NonbondedForce" in self._old_system_forces
            or "NonbondedForce" in self._new_system_forces
        )
        ...
        if has_nonbonded_force:
            ...
            if not (
                len(self._old_system_exceptions.keys()) == 0
                and len(self._new_system_exceptions.keys()) == 0
            ):
    ...
    def _check_and_store_system_forces(self):
        ...
        def _check_unknown_forces(forces, system_name):
            ...
            if unknown_forces:
                errmsg = f"Unknown forces {unknown_forces} encountered in {system_name} system"
        ...
        self._old_system_forces = {
            type(force).__name__: force for force in self._old_system.getForces()
        }
        _check_unknown_forces(self._old_system_forces, "old")
        self._new_system_forces = {
            type(force).__name__: force for force in self._new_system.getForces()
        }
        ...
        * Review influence of lack of mass scaling.
        ...
        for particle_idx in range(self._old_system.getNumParticles()):
            ...
            if particle_idx in self._old_to_new_map.keys():
                ...
                mass_new = self._new_system.getParticleMass(particle_idx_new_system)
    ...
    def _set_atom_classes(self):
        ...
        for new_idx, old_idx in self._core_new_to_old_map.items():
            ...
            if new_to_hybrid_idx != old_to_hybrid_idx:
                errmsg = (
                    f"there is an index collision in hybrid indices of "
        ...
        for new_idx, old_idx in self._env_new_to_old_map.items():
            ...
            if new_to_hybrid_idx != old_to_hybrid_idx:
                errmsg = (
                    f"there is an index collion in hybrid indices of "
    ...
    def _generate_dict_from_exceptions(force):
        ...
        for exception_index in range(force.getNumExceptions()):
            [index1, index2, chargeProd, sigma, epsilon] = force.getExceptionParameters(
                exception_index
            )
    ...
    def _validate_disjoint_sets(self):
        ...
        for old_indices in self._old_system_exceptions.keys():
            hybrid_indices = (
                self._old_to_hybrid_map[old_indices[0]],
            ...
            if old_env_intersection:
                if set(old_indices).intersection(self._atom_classes["unique_old_atoms"]):
                    errmsg = (
                        f"old index exceptions {old_indices} include "
        ...
        for new_indices in self._new_system_exceptions.keys():
            hybrid_indices = (
                self._new_to_hybrid_map[new_indices[0]],
            ...
            if new_env_intersection:
                if set(hybrid_indices).intersection(self._atom_classes["unique_new_atoms"]):
                    errmsg = (
                        f"new index exceptions {new_indices} include "
    ...
    def _handle_constraints(self):
        ...
        for const_idx in range(self._old_system.getNumConstraints()):
            at1, at2, length = self._old_system.getConstraintParameters(const_idx)
        ...
        for const_idx in range(self._new_system.getNumConstraints()):
            at1, at2, length = self._new_system.getConstraintParameters(const_idx)
            hybrid_atoms = tuple(sorted([hybrid_map[at1], hybrid_map[at2]]))
            if hybrid_atoms not in constraint_lengths.keys():
                self._hybrid_system.addConstraint(hybrid_atoms[0], hybrid_atoms[1], length)
                constraint_lengths[hybrid_atoms] = length
        ...
        for particle_idx in range(self._old_system.getNumParticles()):
            if self._old_system.isVirtualSite(particle_idx):
                ...
                if hybrid_idx not in self._atom_classes["environment_atoms"]:
                    errmsg = "Virtual sites in changing residue are unsupported."
                    raise ValueError(errmsg)
                else:
                    virtual_site = self._old_system.getVirtualSite(particle_idx)
                    if isinstance(virtual_site, openmm.ThreeParticleAverageSite):
                        ...
                        errmsg = f"Unsupported VirtualSite class: {virtual_site}"
                        raise ValueError(errmsg)

                    self._hybrid_system.setVirtualSite(hybrid_idx, vs_copy)
            ...
            if self._new_system.isVirtualSite(particle_idx):
                ...
                if hybrid_idx not in self._atom_classes["environment_atoms"]:
                    ...
                    if not self._hybrid_system.isVirtualSite(hybrid_idx):
                        errmsg = (
                            "Environment virtual site in new system "
        ...
        * Move to a dictionary or equivalent.
        ...
        if v2:
            sterics_energy_expression = (
                "U_sterics = select(step(r - r_LJ), 4*epsilon*x*(x-1.0), U_sterics_quad);"
            )
            sterics_energy_expression += (
                "U_sterics_quad = Force*(((r - r_LJ)^2)/2 - (r - r_LJ)) + U_sterics_cut;"
            )
            sterics_energy_expression += (
                "U_sterics_cut = 4*epsilon*((sigma/r_LJ)^6)*(((sigma/r_LJ)^6) - 1.0);"
            )
            sterics_energy_expression += (
                "Force = -4*epsilon*((-12*sigma^12)/(r_LJ^13) + (6*sigma^6)/(r_LJ^7));"
            )
    ...
    def _add_nonbonded_force_terms(self):
        ...
        * Move nonbonded_method defn here to avoid just setting it globally
        ...
        if self._nonbonded_method in [openmm.NonbondedForce.NoCutoff]:
            sterics_energy_expression = self._nonbonded_custom(self._softcore_LJ_v2)
        elif self._nonbonded_method in [
            openmm.NonbondedForce.CutoffPeriodic,
    ...
    def _handle_harmonic_bonds(self):
        ...
        * Bond softening logic has been removed for now.
        ...
        for bond_index in range(old_system_bond_force.getNumBonds()):
            ...
            [index1_old, index2_old, r0_old, k_old] = old_system_bond_force.getBondParameters(
                bond_index
            )
            ...
            if index_set.issubset(self._atom_classes["core_atoms"]):
                ...
                self._hybrid_system_forces["core_bond_force"].addBond(
                    index1_hybrid, index2_hybrid, [r0_old, k_old, r0_new, k_new]
                )
            ...
            #        to understand logic.
                ...
                len(index_set.intersection(self._atom_classes["environment_atoms"])) == 1
                and len(index_set.intersection(self._atom_classes["core_atoms"])) == 1
            ):
                self._hybrid_system_forces["standard_bond_force"].addBond(
                    index1_hybrid, index2_hybrid, r0_old, k_old
            ...
            # same)
                ...
                errmsg = f"hybrid index set {index_set} does not fit into a canonical atom type"
                raise ValueError(errmsg)
        ...
        for bond_index in range(new_system_bond_force.getNumBonds()):
            # Get each set of bond parameters
            [index1_new, index2_new, r0_new, k_new] = new_system_bond_force.getBondParameters(
                bond_index
            )
            ...
            if len(index_set.intersection(self._atom_classes["unique_new_atoms"])) == 2 or (
                len(index_set.intersection(self._atom_classes["unique_new_atoms"])) == 1
                and len(index_set.intersection(self._atom_classes["core_atoms"])) == 1
            ):
            ...
            # TODO - Work out what the above peculiarities are...
                ...
                if not self._find_bond_parameters(
                    self._hybrid_system_forces["core_bond_force"], index1_hybrid, index2_hybrid
                ):
                    r0_old = r0_new
    ...
    def _find_angle_parameters(angle_force, indices):
        ...
        for angle_index in range(angle_force.getNumAngles()):
            ...
            if indices == angle_param_indices or indices_reversed == angle_param_indices:
                return angle_params
        ...
        * Removed softening and neglected angle functionality
        ...
        for angle_index in range(old_system_angle_force.getNumAngles()):
            ...
            # which would mean they involve unique old interactions
                ...
                if hybrid_index_set.intersection(self._atom_classes["environment_atoms"]):
            ...
            # add it to the standard angle force
                ...
                errmsg = (
                    f"handle_harmonic_angles: angle_index {angle_index} "
        ...
        for angle_index in range(new_system_angle_force.getNumAngles()):
            ...
            if len(hybrid_index_set.intersection(self._atom_classes["unique_new_atoms"])) > 0:
                if hybrid_index_set.intersection(self._atom_classes["environment_atoms"]):
                    errmsg = "we disallow angle terms with unique new and environment atoms"
                    raise ValueError(errmsg)
                ...
                if not self._find_angle_parameters(
                    self._hybrid_system_forces["core_angle_force"], hybrid_index_list
                ):
                    hybrid_force_parameters = [
                        new_angle_parameters[3],
                        0.0 * unit.kilojoule_per_mole / unit.radian**2,
                ...
                errmsg = (
                    f"hybrid index list {hybrid_index_list} does not fit into a canonical atom set"
                )
                raise ValueError(errmsg)
    ...
    def _find_torsion_parameters(torsion_force, indices):
        ...
        for torsion_idx in range(torsion_force.getNumTorsions()):
            ...
            if indices == torsion_param_indices or indices_reversed == torsion_param_indices:
    ...
    def _handle_nonbonded(self):
        ...
        * A lot of this logic is duplicated, probably turn it into a couple of
        ...
        self._hybrid_system_forces["standard_nonbonded_force"].addGlobalParameter(
        ...
        for particle_index in range(self._hybrid_system.getNumParticles()):
            if particle_index in self._atom_classes["unique_old_atoms"]:
                ...
                [charge, sigma, epsilon] = old_system_nonbonded_force.getParticleParameters(
                    old_index
                )
                ...
                self._hybrid_system_forces["standard_nonbonded_force"].addParticleParameterOffset(
                    "lambda_electrostatics_delete", particle_index, -charge, 0 * sigma, 0 * epsilon
                )
                ...
                [charge, sigma, epsilon] = new_system_nonbonded_force.getParticleParameters(
                    new_index
                )
                ...
                self._hybrid_system_forces["standard_nonbonded_force"].addParticleParameterOffset(
                    "lambda_electrostatics_insert", particle_index, +charge, 0, 0
                )

            elif particle_index in self._atom_classes["core_atoms"]:
                # Get the parameters in the new and old systems:
                old_index = hybrid_to_old_map[particle_index]
                [charge_old, sigma_old, epsilon_old] = (
                    old_system_nonbonded_force.getParticleParameters(old_index)
                )
                new_index = hybrid_to_new_map[particle_index]
                [charge_new, sigma_new, epsilon_new] = (
                    new_system_nonbonded_force.getParticleParameters(new_index)
                )
                ...
                self._hybrid_system_forces["standard_nonbonded_force"].addParticleParameterOffset(
                    "lambda_electrostatics_core", particle_index, (charge_new - charge_old), 0, 0
                )
    ...
    def _handle_hybrid_exceptions(self):
        ...
        for atom_pair in unique_old_pairs:
            ...
            # Check if the pair is in the reverse order and use that if so
                ...
                [chargeProd, sigma, epsilon] = self._old_system_exceptions[
                    old_index_atom_pair[::-1]
                ]
                # If we are interpolating 1,4 exceptions then we have to
                if self._interpolate_14s:
                    self._hybrid_system_forces["standard_nonbonded_force"].addException(
        ...
        for atom_pair in unique_new_pairs:
            ...
            # Check if the pair is present in the reverse order and use that if so
                ...
                [chargeProd, sigma, epsilon] = self._new_system_exceptions[
                    new_index_atom_pair[::-1]
                ]
                if self._interpolate_14s:
                    self._hybrid_system_forces["standard_nonbonded_force"].addException(
                        atom_pair[0], atom_pair[1], chargeProd * 0.0, sigma, epsilon * 0.0
        ...
        for exception_pair, exception_parameters in self._old_system_exceptions.items():
            ...
            # the other (new) system, and interpolate between the two
                ...
                if not new_exception_parms:
                    [charge1_new, sigma1_new, epsilon1_new] = (
                        new_system_nonbonded_force.getParticleParameters(index1_new)
                    )
                    [charge2_new, sigma2_new, epsilon2_new] = (
                        new_system_nonbonded_force.getParticleParameters(index2_new)
                    )
                    ...
                    [index1_new, index2_new, chargeProd_new, sigma_new, epsilon_new] = (
                        new_exception_parms
                    )

                # Interpolate between old and new
                exception_index = self._hybrid_system_forces[
                    "standard_nonbonded_force"
                ].addException(index1_hybrid, index2_hybrid, chargeProd_old, sigma_old, epsilon_old)
                self._hybrid_system_forces["standard_nonbonded_force"].addExceptionParameterOffset(
                    "lambda_electrostatics_core",
                    exception_index,
                    (chargeProd_new - chargeProd_old),
                    0,
                    0,
                )
                self._hybrid_system_forces["standard_nonbonded_force"].addExceptionParameterOffset(
        ...
        for exception_pair, exception_parameters in self._new_system_exceptions.items():
            ...
            # but not the other (ring closure)
                ...
                if not self._find_exception(old_system_nonbonded_force, index1_old, index2_old):
                    [charge1_old, sigma1_old, epsilon1_old] = (
                        old_system_nonbonded_force.getParticleParameters(index1_old)
                    )
                    [charge2_old, sigma2_old, epsilon2_old] = (
                        old_system_nonbonded_force.getParticleParameters(index2_old)
                    )
    ...
    def _handle_old_new_exceptions(self):
        ...
        * Move old_new_bond_exceptions to a dictionary or similar.
        ...
        if self._softcore_LJ_v2:
            ...
            old_new_nonbonded_exceptions += (
                f"U_sterics_quad = Force*(((r - r_LJ)^2)/2 - (r - r_LJ)) + U_sterics_cut;"
            )
            old_new_nonbonded_exceptions += (
                f"U_sterics_cut = 4*epsilon*((sigma/r_LJ)^6)*(((sigma/r_LJ)^6) - 1.0);"
            )
            old_new_nonbonded_exceptions += (
                f"Force = -4*epsilon*((-12*sigma^12)/(r_LJ^13) + (6*sigma^6)/(r_LJ^7));"
            )
            old_new_nonbonded_exceptions += f"x = (sigma/r)^6;"
            old_new_nonbonded_exceptions += (
                f"r_LJ = softcore_alpha*((26/7)*(sigma^6)*lambda_sterics_deprecated)^(1/6);"
            )
        ...
        old_new_nonbonded_exceptions += (
            "epsilon = (1-lambda_sterics)*epsilonA + lambda_sterics*epsilonB;"  # interpolation
        )
        ...
        old_new_nonbonded_exceptions += (
            "new_interaction = delta(1-unique_new); old_interaction = delta(1-unique_old);"
        )
        ...
        nonbonded_exceptions_force.addGlobalParameter("lambda_electrostatics_insert", 0.0)
        # electrostatics delete
        nonbonded_exceptions_force.addGlobalParameter("lambda_electrostatics_delete", 0.0)
        # sterics insert
        nonbonded_exceptions_force.addGlobalParameter("lambda_sterics_insert", 0.0)
        # steric delete
        nonbonded_exceptions_force.addGlobalParameter("lambda_sterics_delete", 0.0)
        ...
        for exception_pair, exception_parameters in self._old_system_exceptions.items():
            ...
            if len(index_set.intersection(self._atom_classes["unique_old_atoms"])) > 0 and (
                ...
                or epsilon_old.value_in_unit_system(unit.md_unit_system) != 0.0
            ):
                if self._interpolate_14s:
        ...
        for exception_pair, exception_parameters in self._new_system_exceptions.items():
            ...
            if len(index_set.intersection(self._atom_classes["unique_new_atoms"])) > 0 and (
                ...
                or epsilon_new.value_in_unit_system(unit.md_unit_system) != 0.0
            ):
                if self._interpolate_14s:
                    # If we are interpolating 1,4s, then we anneal this term
    ...
    def _compute_hybrid_positions(self):
        ...
        old_pos_without_units = np.array(self._old_positions.value_in_unit(unit.nanometer))
        new_pos_without_units = np.array(self._new_positions.value_in_unit(unit.nanometer))
    ...
    def _create_mdtraj_topology(self):
        ...
        * A lot of this can be simplified / reworked.
        ...
        for particle_idx in self._unique_new_atoms:
            ...
            # new to old atom map
            mapped_old_atom_indices = [
                self._new_to_old_map[atom_idx] for atom_idx in mapped_new_atom_indices
            ]
            ...
            added_atoms[new_particle_hybrid_idx] = hybrid_topology.add_atom(
                new_system_atom.name, new_system_atom.element, mapped_residue
            )
        ...
        * Unlike the MDTraj Topology object, the residues of the alchemical
        ...
        for at in atom_list:
            ...
            if at.residue != prev_res:
                hybrid_residue = hybrid_top.addResidue(at.residue.name, hybrid_chain, at.residue.id)
                prev_res = at.residue
