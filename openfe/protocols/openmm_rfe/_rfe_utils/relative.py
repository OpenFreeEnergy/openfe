...
class HybridTopologyFactory:
    ...
    * Implement omitted terms (this was not available in the original class).
    ...
    def __init__(
        ...
        has_nonbonded_force = "NonbondedForce" in self._old_system_forces or "NonbondedForce" in self._new_system_forces
        ...
        if has_nonbonded_force:
            ...
            if not (len(self._old_system_exceptions.keys()) == 0 and len(self._new_system_exceptions.keys()) == 0):
    ...
    def _check_and_store_system_forces(self):
        ...
        def _check_unknown_forces(forces, system_name):
            ...
            if unknown_forces:
                errmsg = f"Unknown forces {unknown_forces} encountered in " f"{system_name} system"
    ...
    def _validate_disjoint_sets(self):
        ...
        for old_indices in self._old_system_exceptions.keys():
            ...
            old_env_intersection = set(old_indices).intersection(self._atom_classes["environment_atoms"])
        ...
        for new_indices in self._new_system_exceptions.keys():
            ...
            new_env_intersection = set(hybrid_indices).intersection(self._atom_classes["environment_atoms"])
            ...
            if self._old_system.isVirtualSite(particle_idx):
                ...
                if hybrid_idx not in self._atom_classes["environment_atoms"]:
                    ...
                    if isinstance(virtual_site, openmm.ThreeParticleAverageSite):
                        ...
                        errmsg = "Unsupported VirtualSite " f"class: {virtual_site}"
                        raise ValueError(errmsg)
            ...
            if self._new_system.isVirtualSite(particle_idx):
                ...
                if hybrid_idx not in self._atom_classes["environment_atoms"]:
                    ...
                    if not self._hybrid_system.isVirtualSite(hybrid_idx):
                        errmsg = "Environment virtual site in new system " "found not copied from old system"
                        raise ValueError(errmsg)
    ...
    def _nonbonded_custom_sterics_common():
        ...
        * Move to a dictionary or equivalent.
        ...
        sterics_addition += (
            "lambda_alpha = new_interaction*(1-lambda_sterics_insert) + old_interaction*lambda_sterics_delete;"
        )
    ...
    def _add_nonbonded_force_terms(self):
        ...
        * Move nonbonded_method defn here to avoid just setting it globally
        ...
        if self._softcore_LJ_v2:
            ...
            sterics_custom_nonbonded_force.addGlobalParameter("softcore_alpha", self._softcore_alpha)
        ...
        * Removed softening and neglected angle functionality
        ...
        for angle_index in range(old_system_angle_force.getNumAngles()):
            ...
            if hybrid_index_set.issubset(self._atom_classes["core_atoms"]):
                ...
                new_indices = [self._old_to_new_map[old_atomid] for old_atomid in old_angle_parameters[:3]]
            ...
            # add it to the standard angle force
                ...
                errmsg = f"handle_harmonic_angles: angle_index {angle_index} " "does not fit a canonical form."
                raise ValueError(errmsg)
        ...
        for angle_index in range(new_system_angle_force.getNumAngles()):
            ...
            if len(hybrid_index_set.intersection(self._atom_classes["unique_new_atoms"])) > 0:
                ...
                errmsg = f"hybrid index list {hybrid_index_list} does not " "fit into a canonical atom set"
                raise ValueError(errmsg)
    ...
    def _handle_periodic_torsion_force(self):
        ...
        * Torsion flattening logic has been removed for now.
        ...
        for index in [
            q for q in range(len(auxiliary_custom_torsion_force)) if q not in old_custom_torsions_to_standard
        ]:
        ...
        for exception_pair, exception_parameters in self._old_system_exceptions.items():
            ...
            # the other (new) system, and interpolate between the two
                ...
                if not new_exception_parms:
                    [charge1_new, sigma1_new, epsilon1_new] = new_system_nonbonded_force.getParticleParameters(
                        index1_new,
                    )
                    [charge2_new, sigma2_new, epsilon2_new] = new_system_nonbonded_force.getParticleParameters(
                        index2_new,
                    )
        ...
        for exception_pair, exception_parameters in self._new_system_exceptions.items():
            ...
            # but not the other (ring closure)
                ...
                if not self._find_exception(old_system_nonbonded_force, index1_old, index2_old):
                    ...
                    [charge1_old, sigma1_old, epsilon1_old] = old_system_nonbonded_force.getParticleParameters(
                        index1_old,
                    )
                    [charge2_old, sigma2_old, epsilon2_old] = old_system_nonbonded_force.getParticleParameters(
                        index2_old,
                    )
    ...
    def _handle_old_new_exceptions(self):
        ...
        * Move old_new_bond_exceptions to a dictionary or similar.
        ...
        old_new_nonbonded_exceptions += (
            "lambda_sterics = new_interaction*lambda_sterics_insert + old_interaction*lambda_sterics_delete;"
        )
            ...
            if len(index_set.intersection(self._atom_classes["unique_old_atoms"])) > 0 and (
                ...
                if self._interpolate_14s:
                    ...
                    nonbonded_exceptions_force.addBond(
                        index1_hybrid,
                        index2_hybrid,
                        [chargeProd_old, sigma_old, epsilon_old, sigma_old, epsilon_old * 0.0, 1, 0],
                    )
            ...
            if len(index_set.intersection(self._atom_classes["unique_new_atoms"])) > 0 and (
                ...
                if self._interpolate_14s:
                    ...
                    nonbonded_exceptions_force.addBond(
                        index1_hybrid,
                        index2_hybrid,
                        [chargeProd_new, sigma_new, epsilon_new * 0.0, sigma_new, epsilon_new, 0, 1],
                    )
    ...
    def _create_mdtraj_topology(self):
        ...
        * A lot of this can be simplified / reworked.
        ...
        for particle_idx in self._unique_new_atoms:
            ...
            # new to old atom map
            mapped_old_atom_indices = [self._new_to_old_map[atom_idx] for atom_idx in mapped_new_atom_indices]
    ...
    def _create_hybrid_topology(self):
        ...
        * Unlike the MDTraj Topology object, the residues of the alchemical
        ...
        for bond in self._new_topology.bonds():
            ...
            if (at1 in self._atom_classes["unique_new_atoms"]) or (at2 in self._atom_classes["unique_new_atoms"]):
                hybrid_top.addBond(
                    list(hybrid_top.atoms())[at1],
                    list(hybrid_top.atoms())[at2],
                    bond.type,
