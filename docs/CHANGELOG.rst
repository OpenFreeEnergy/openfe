...
* The ``RelativeHybridTopologyProtocol``, ``AbsoluteBindingProtocol``, ``AbsoluteSolvationProtocol``, and ``SepTopProtocol`` now return an unbiased estimate of the standard deviation via the ``get_uncertainty`` method, reported uncertainties are expected to be larger than before (`PR #2000 <https://github.com/OpenFreeEnergy/openfe/pull/2000>`_).
